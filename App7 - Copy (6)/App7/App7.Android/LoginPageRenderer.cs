...
namespace App7.Droid
{
    ...
    [Activity(NoHistory  = true, Label = "Asisto", ConfigurationChanges = ConfigChanges.Orientation | ConfigChanges.ScreenSize,LaunchMode = LaunchMode.SingleInstance)]
            ...
            auth.Completed += (sender, eventArgs) => {
                if (eventArgs.IsAuthenticated)
                {
                    ...
                    var request = new OAuth2Request("GET",
                        new Uri(
                            "https://graph.facebook.com/me?fields=email,first_name,last_name,gender,picture,age_range,cover,id,timezone,locale,link,verified")
                            , null, account);
                    ...
                    Task.WhenAll(x);//required to prevent unknown error exeption during retrival
                    ...
                    x.ContinueWith(
                        t =>
                        {
                            ...
                            else
                            {
                                ...
                                var m = JsonConvert.DeserializeObject<ViewModels.UserInfoModel>(json);
                                //UserInfo is a static object 
                                  new UserInfo().UserInfoModel = m;//the instance must be the one being used upon creation
                                                                   //you shoun't retrive the image url here you can run it over a task run or at least a delay
                                                                   //   Console.WriteLine(UserInfo.Instance.UserInfoModel.id + " "+UserInfo.Instance.UserInfoModel.first_name +" "+UserInfo.Instance.UserInfoModel.last_name);

                                CrossSecureStorage.Current.SetValue("UserID", m.id);

                                if (CrossSecureStorage.Current.HasKey("UserID"))//making sure its created
                                {

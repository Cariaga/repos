...
namespace App7
{
    public class Browser
    {
        ...
        public async Task<T> JsonWebAsync<T>(string Url)
        {
 ...
 //  @"^[0-9]{3}-[0-9]{4}-[0-9]{4}$",
    ...
    public partial class MainPage : ContentPage
    {
        ...
        public MainPage()
        {
            ...
            Task.Run(async () => {
               var x= new Random();
               var SetLocationRequestState =  new Controllers.ServerLocationController().SetLocationByUser("UserName2",x.NextDouble().ToString(),x.NextDouble().ToString());
               var GetLocationByUser= new Controllers.ServerLocationController().GetLocationByUser("UserName2", "fcb6cec6-e7d4-11e7-87ce-00163e4a7907");


                await Task.WhenAll(SetLocationRequestState,GetLocationByUser);

                var LocalNetwork = GetLocationByUser.Result.Where(f => f.NetworkIdentity == "Local").FirstOrDefault();
                var ClientNetwork = GetLocationByUser.Result.Where(f => f.NetworkIdentity == "Client").FirstOrDefault();


                Debug.WriteLine(SetLocationRequestState.Result.ToString() +" "+GetLocationByUser.Result.First() +" "+ LocalNetwork.NetworkIdentity + " "+ClientNetwork.NetworkIdentity);
            });
            */
            ...
            Task.Run(async () => {
               ...
               var DestinationRequestState =  new Controllers.ServerTaskController().SetDestination("24", "10", "fcb6cec6-e7d4-11e7-87ce-00163e4a7907", "UserName2");
               var OriginRequestState =  new Controllers.ServerTaskController().SetOrigin("24", "10", "fcb6cec6-e7d4-11e7-87ce-00163e4a7907", "UserName2");

              

                //--this 3 will set it simultaniously so you won't notice it happend
                var OngoingRequestState =  new Controllers.ServerTaskController().SetOngoing("fcb6cec6-e7d4-11e7-87ce-00163e4a7907", "UserName2");
                var CancelledRequestState =  new Controllers.ServerTaskController().SetCancelled("fcb6cec6-e7d4-11e7-87ce-00163e4a7907", "UserName2");
                var CompletedRequestState =  new Controllers.ServerTaskController().SetCompleted("fcb6cec6-e7d4-11e7-87ce-00163e4a7907", "UserName2");

                await Task.WhenAll(DestinationRequestState, OriginRequestState, OngoingRequestState, CancelledRequestState, CompletedRequestState);

                Debug.WriteLine(DestinationRequestState.Result.ToString() +" "+OriginRequestState.Result.ToString()+" "+CancelledRequestState.Result.ToString()+" "+CompletedRequestState.Result.ToString());
            });
            ...
            **/
            Task.Run(() =>
            {
                var EmailExist = new Controllers.ServerAccountController().IsEmailExistAsync("Email");
                var PhoneNumberExist = new Controllers.ServerAccountController().IsNumberExistAsync("PhoneNumber");
                var UserNameExist = new Controllers.ServerAccountController().IsUserNameExistAsync("UserName");
                Task.WhenAll(EmailExist, UserNameExist, PhoneNumberExist);
            //    Debug.WriteLine(EmailExist.Result + " " + UserNameExist.Result + " " + PhoneNumberExist.Result);
